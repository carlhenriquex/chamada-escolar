...
if ($turma === '') {
    redirecionar("Turma não especificada.", $turma);
}

if (!isset($_POST["notas"])) {
    redirecionar("Nenhuma nota foi enviada.", $turma);
}

$professorId = $_SESSION["id"] ?? null;

if (!$professorId) {
    redirecionar("Sessão inválida. Faça login novamente.", $turma);
}
...
if (!$row || empty($row["disciplina"])) {
    redirecionar("Erro ao obter a disciplina do professor.", $turma);
}
...
$stmt->close();
redirecionar("Notas salvas com sucesso.", $turma);